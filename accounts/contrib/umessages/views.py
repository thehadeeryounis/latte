...
from accounts.contrib.umessages.models import Message, MessageRecipient, MessageContact
from accounts.contrib.umessages.forms import ComposeForm
from accounts.utils import get_datetime_now
...
def message_compose(request, recipients=None, compose_form=ComposeForm,
    ...
    :param success_url:
        ...
        sending a message. Defaults to ``accounts_umessages_list`` if there are
        multiple recipients. If there is only one recipient, will redirect to
        ``accounts_umessages_detail`` page, showing the conversation.
...
def message_remove(request, undo=False):
    ...
    else: return redirect(reverse('accounts_umessages_list'))
