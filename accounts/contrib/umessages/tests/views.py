...
from accounts.contrib.umessages.forms import ComposeForm
from accounts.contrib.umessages.models import Message, MessageRecipient
...
class MessagesViewsTests(TestCase):
    ...
    def test_compose(self):
        ...
        # Login is required.
        self._test_login('accounts_umessages_compose')

        # Sign in
        client = self.client.login(username='john', password='blowfish')
        response = self.client.get(reverse('accounts_umessages_compose'))
    ...
    def test_compose_post(self):
        ...
        response = self.client.post(reverse('accounts_umessages_compose'),
                                    data=valid_data)

        self.assertRedirects(response,
                             reverse('accounts_umessages_detail',
                                     kwargs={'username': 'john'}))

        # Check for a requested redirect
        valid_data['next'] = reverse('accounts_umessages_compose')
        response = self.client.post(reverse('accounts_umessages_compose'),
                                    data=valid_data)
        self.assertRedirects(response,
                             valid_data['next'])
    ...
    def test_compose_recipients(self):
        ...
        response = self.client.get(reverse('accounts_umessages_compose_to',
    ...
    def test_message_detail(self):
        ...
        self._test_login('accounts_umessages_detail',
                          kwargs={'message_id': 2})

        # Sign in
        client = self.client.login(username='jane', password='blowfish')
        response = self.client.get(reverse('accounts_umessages_detail',
                                   kwargs={'message_id': 1}))
    ...
    def test_valid_message_remove(self):
        ...
        response = self.client.post(reverse('accounts_umessages_remove'))
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(reverse('accounts_umessages_remove'))
        ...
        response = self.client.post(reverse('accounts_umessages_remove'),
                                    data={'message_pks': '1'})
        self.assertRedirects(response,
                             reverse('accounts_umessages_list'))
        ...
        response = self.client.post(reverse('accounts_umessages_remove'),
                                    data={'message_pks': '1',
                                          'next': reverse('accounts_umessages_list')})
        self.assertRedirects(response,
                             reverse('accounts_umessages_list'))
        jane = User.objects.get(username='jane')
    ...
    def test_message_unremove(self):
        ...
        response = self.client.post(reverse('accounts_umessages_unremove'),
                                    data={'message_pks': [1,]})
    ...
    def test_message_list(self):
        ...
        self._test_login("accounts_umessages_list")

        client = self.client.login(username="john", password="blowfish")
        response = self.client.get(reverse("accounts_umessages_list"))
        self.assertEqual(response.status_code, 200)
