...
from accounts.utils import generate_sha1, get_profile_model, get_datetime_now
...
class UserenaManager(UserManager):
    ...
    def create_user(self, username, email, password, active=False,
                    send_email=True, override = False, user = None):
        """
    ...
    def activate_user(self, username, activation_key):
        ...
        if SHA1_RE.search(activation_key):
            try:
                accounts = self.get(user__username=username,
                                   activation_key=activation_key)
