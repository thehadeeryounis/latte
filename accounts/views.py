...
from accounts.forms import (SignupForm, SignupFormOnlyEmail, AuthenticationForm,
                           ChangeEmailForm, EditProfileForm)
from accounts.models import UserenaSignup
from accounts.decorators import secure_required
from accounts.backends import UserenaAuthenticationBackend
...
def signup(request, signup_form=SignupForm,
    ...
    **Context**
    ...
    if accounts_settings.ACCOUNTS_WITHOUT_USERNAMES and (signup_form == SignupForm):
...
def activate(request, username, activation_key,
    ...
    if user:
        ...
        else: redirect_to = reverse('accounts_profile_detail',
                                    kwargs={'username': user.username})
        return redirect(redirect_to)
    else:
        if not extra_context: extra_context = dict()
...
def email_confirm(request, username, confirmation_key,
    ...
    if user:
        if success_url: redirect_to = success_url
        else: redirect_to = reverse('accounts_email_confirm_complete',
                                    kwargs={'username': user.username})
        return redirect(redirect_to)
    else:
        if not extra_context: extra_context = dict()
...
def profile_detail(
    ...
    **Context**
    ...
    try:
        ...
        UserenaSignup.objects.create_user(username, user.email, "lol", active=True,send_email=False, override = True, user = user)
        profile = user.get_profile()
