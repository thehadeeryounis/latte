...
class GenericViews(BaseViews):
    ...
    def add_success(self, request, instance):
        action.send(request.user, verb='created '+self.model._meta.object_name.lower(), action_object=request.user, target = instance)
        if request.is_ajax():
    ...
    def edit_success(self, request, instance):
        action.send(request.user, verb='edited '+self.model._meta.object_name.lower(), action_object=request.user, target = instance)
        if request.is_ajax():
            return self.render_to_json({'status': 'OK'})
        else:
            return redirect(instance.get_absolute_url())
    ...
    ### Index view
    def index_view(self, request, template="smarter/index.html", extra_context=None, page_template="smarter/index_page.html"):
        self.check_permissions()
        if request.is_ajax():
            template = page_template
        return render_to_response(template,{'page_template': page_template,'objects_list': self.model.objects.all()}, context_instance=RequestContext(request))
    ### Object view
    ...
    def remove_success(self, request, obj):
        action.send(request.user, verb='deleted '+self.model._meta.object_name.lower(), action_object=request.user, target = instance)
        if request.is_ajax():
            return self.render_to_json({'status': 'OK'})
        else:
            return redirect('../..')
